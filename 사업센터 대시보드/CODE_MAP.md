...
- **표 구성은 사용자 스펙 그대로다** — 「월별 실적 현황」은 **플러스 가입 목표 / 플러스 가입 실적 /
  └ 재가입 / 달성 실적 / 달성률 + 개업세무사 4행**(v08.299 기준). `└ 재가입`은 **플러스 가입 실적
  안에 든 재가입분**(`joinSplitCount('plus',s,e,true)`)이라 합계에 또 더하지 않는다.
  ※ 이 행은 v08.297에서 「스펙에 없다」는 지적으로 지웠다가 v08.299에서 사용자가 다시 넣었다 —
