...
| **개업세무사** | 모수 중 **프로모션 구분명**이 개업세무사 성장지원 패키지인 곳 → `window.pkgPromoOf(x)`(`isPromo190`) | 모수와 **동일**(가입 기준) |
...
- **개업세무사는 플러스의 부분집합**이다(같은 모수·같은 날짜 기준) — 그래서 `└` 하위 표기가 정확하고
  하위가 상위보다 커질 수 없다. **합계에는 더하지 않는다**(플러스에 이미 포함).
