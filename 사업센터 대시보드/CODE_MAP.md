...
**화면을 새로 만들거나, 화면이 다른 함수로 옮겨가거나, 아래 표의 내용이 바뀌는 수정을 하면
그 자리에서 이 문서도 함께 갱신한다.** 안 그러면 지도가 낡아서 오히려 헷갈리게 만든다.
반대로 사소한 문구·색상 수정처럼 "담당 함수"가 안 바뀌는 경우는 이 문서를 건드릴 필요 없다.
