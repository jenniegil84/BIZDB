...
## 13. 「가입」 집계 공통 기준 (현재 = v08.302)

**두 표는 역할이 다르고 모수도 다르다**(v08.300) — `window.joinSplitCount(kind,s,e,onlyRejoin,scope)`의
`scope`로 갈린다.

| 표 | 역할 | 모수(scope) | 2026 합계(실데이터) |
|---|---|---|---|
| 「플러스 가입 및 프로모션 가입 — 월별 실적 현황」(`monPerfTableHtml`) | **플러스 집계** | 기본(생략) = 신규가입 + 재가입 + 플러스 전환(요금제변경) + 인사이트 추가 가입 | 플러스 **102** |
| 「전체 가입 현황」(`planJoinTableHtml`) | **전체 집계** | `'join'` = 그 달의 신규가입 + 재가입(모든 요금제) | 플러스 34 · 리포트 58 · 합계 **92** |
...
### 13-1. 카드 밑줄(모수 내역)과 표 3줄(실적 분해)은 다른 숫자다 — 정상

실적 분석 카드(`perfStat` → `plusNote`)의 밑줄은 **모수 27건의 경로별 내역**이고,
주간회의 표의 세부 3줄은 **실적 20건을 겹치지 않게 쪼갠 것**이다. 총 숫자(플러스 가입)는
두 화면이 같지만 세부 숫자는 다르게 보이는 게 정상이다(2026-08 실데이터로 확인).

| 화면 | 숫자 | 뜻 |
|---|---|---|
| 카드 밑줄 | 가입 27건(신규 9 · 재가입 1 · 플러스 전환 13 · 인사이트 신규 19) | **모수**의 경로별 원건수 — 인사이트 유무를 보지 않고, 한 곳이 여러 경로에 겹쳐 잡힌다(합 42 → 중복 제거 27) |
| 카드 숫자 | 플러스 가입 20건 | 모수 27곳 중 `insJn`이 있는 곳 |
| 표 세부 3줄 | 신규 플러스 2 · 재가입 1 · 업셀 17 | **20건**의 상호배타 분해(우선순위 신규>재가입>업셀) |
...
두 표 아래에 각각 기준 설명 문구가 있다 — 숫자가 달라 보이는 이유를 화면에서 읽을 수 있어야 한다.
