...
그 아래 **「└ 그중 개업세무사 (프로모션)」** 줄(v08.302)은 세부 3줄과 **성격이 다르다** —
개업세무사는 플러스 요금제에 얹히는 프로모션이라 위 3줄 어디에나 걸쳐 있고, 값은
`pkgStartCount`(프로모션 시작월)라 **3줄의 합에 더하지 않는다**. 이 줄을 넣은 이유는
「아래 파란 블록(개업세무사 목표·실적)이 위 플러스 실적에 포함된 건지 헷갈린다」는 지적
(2026-09-09) 때문이다 — 파란 블록 라벨도 「개업세무사 가입 목표 **(플러스 내)**」로 적어
포함 관계를 화면에서 읽을 수 있게 했다. 두 값은 항상 같아야 한다(2026 실데이터 13 = 13).
...
- **표 구성은 사용자 스펙 그대로다** — 「월별 실적 현황」은 **플러스 가입 목표 / 플러스 가입 실적 /
  └ 신규 플러스 / └ 재가입 / └ 업셀(인사이트·전환) / └ 그중 개업세무사 (프로모션) / 달성 실적 /
  달성률 + 개업세무사 4행**(v08.302 기준). `└` 줄은 모두 **플러스 가입 실적 안의 내역**이라
