...
## 13. 「가입」을 세 갈래로 나누는 공통 기준 (v08.287 → v08.288)

**v08.288 최종 규칙(사용자 스펙)** — 서로 겹치지 않는 3분류, 계산은 `window.joinSplitCount(kind,s,e,onlyRejoin)`
한 함수만 쓴다(`kind` = `'total'|'pkg'|'plus'|'basic'`).

| 갈래 | 판정 | 원천 |
|---|---|---|
| **개업세무사** | 청구 파일의 **프로모션 구분값**이 개업세무사 성장지원 패키지(=개업성장패키지) → `window.pkgPromoOf(x)` / `isPromo190` | 청구대상 고객관리 파일(`__billAllByBiz`/`__billAllByName`, 상태 무관) |
| **플러스** | 개업세무사가 **아니고** 인사이트 가입일자(`insJn`)가 있음 | 고객 마스터 |
| **베이직** | 개업세무사가 아니고 `insJn`이 없음 | 고객 마스터 |
| **합계** | 그 기간에 가입일 또는 재가입일이 찍힌 곳 전체 = 위 셋의 합 | 고객 마스터 |

- **기간 기준은 세 갈래 모두 가입일·재가입일**이다. 종전에 개업성장패키지만 청구 파일의
  「프로모션 시작월」로 세어 다른 행과 기간 귀속이 달랐고(2026-08 플러스 3 < 패키지 5),
  2026-09-09 결정으로 가입일 기준으로 통일했다. 프로모션 시작월 기준 값이 필요하면
  `pkgJoinCountRange`(그대로 남아 있음)를 쓴다 — 실적 분석 카드 밑줄에 참고로 병기한다.
- 청구 파일이 없으면 개업세무사는 **판정 불가 → null(화면 「—」)**, 플러스·베이직은 `insJn`만으로 가른다.
